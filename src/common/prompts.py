...
14. 실제로 존재하지 않는 관광지, 행사, 건물, 마을 이름은 절대 언급하지 마라.
15. 모르는 장소는 임의로 만들지 말고 '정보 없음'으로 표시해라.
16. 반드시 실제 존재하는 한국의 명소, 문화재, 관광지만 포함해라.
17. 역사적 사건, 감옥/교도소, 정부기관, 군사시설 등 관광 목적이 아닌 장소는 절대 추천하지 않는다.
18. 장소 후보 중 잘못된 장소(비여행지)가 포함되어 있으면 무시하고, 그 사실을 [주의사항]에만 간단히 언급한다.
...
<density-rule>
1. 장소가 1~3개일 경우 → 각 장소 설명과 기념품, 이동 힌트, 역사/문화적 맥락, 소소한 팁을 **풍성하게 자세히** 제공하라.
2. 항상 전체 글 분량은 **시각적으로 균형** 있어 보이도록 조절하라.
3. 전체 이동 경로의 지역 순서와 상세 일정의 순서를 철저하게 지켜라.
4. 이동 수단은 최대한 이동 시간이 짦은 것만 얘기할 것 
5. 상세 일정의 각 장소마다 다음 이동을 표시한다. 
6. 상세 일정의 마지막 장소의 {{다음 이동}} 정보는 뜨지 않는다.
7. 상세 일정의 장소 설명 갯수와 선택한 장소 갯수와 반드시 맞아야함.
8. [규칙] <> 안의 옵션 중 1개 또는 2개를 선택해서 쉼표로 구분하여 출력할 것.
9. 체류 시간은 아래 내용을 참고해서 진행한다.
----
{remaind_times}
----
10. 이동 경로는 아래 내용을 참고해서 진행 한다.
----
{move_times}
----
</density-rule>
